...
def preprocess_sentence(sentence, answer):
    ...
    for token in sentence_doc:
        ent = token.ent_type_
        text = token.text
        if ent != "" and text != "" and text != " ":
            #print("replace -"+text+"-")
            sentence = sentence.replace(text," "+str(ent)+" "+str(i)+" ")
            answer = answer.replace(text," "+str(ent)+" "+str(i)+" ")
        i+=1
...
def test_step(inp, targ, ans, enc_hidden):
    loss = 0

    enc_output, enc_hidden = encoder(inp, enc_hidden)
    dec_hidden = enc_hidden
    dec_input = tf.expand_dims([0] * BATCH_SIZE,1)
    #dec_input = tf.expand_dims([tokenizer.encode("[CLS]")] * BATCH_SIZE, 1)
...
if train_flag == 1:
    ...
    #test
      # for (batch, (inp, targ, ans)) in enumerate(dataset_test.take(steps_per_epoch)):
      #   test_step(inp, targ, ans, enc_hidden)
...
def evaluate(sentence, answer):
  attention_plot = np.zeros((max_length_targ, max_length_inp))

  #in preprocessing
  sentence, answer = preprocess_sentence(sentence, answer)

  sentence = re.sub(' +',' ',sentence)
  sentence = sentence.lower()


  inputs = tensor(sentence, tokenizer)
...
def translate(sentence, answer):
  result, sentence, attention_plot = evaluate(sentence, answer)
...
if train_flag == 0:
    ...
    ###test###
    #translate(u'victoria has a written constitution enacted in 1975', u'constitution')

    translate(u'william of Normandy is England', u'william')
